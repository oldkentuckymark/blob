...
namespace stn {
	...
	namespace impl {
		...
		inline constexpr void* memmove_constexpr(std::uint32_t* pDest, const std::uint32_t* pSrc, std::size_t sSize) {
		...
		inline constexpr unsigned char BitScanReverse_constexpr(unsigned long* pIndex, unsigned long ulMask) {
			if (std::is_constant_evaluated()) {
				if (ulMask == 0) {
					return 0;
				}
		...
		[[nodiscard]] inline constexpr Big_integer_flt Make_big_integer_flt_one() noexcept {
		...
		[[nodiscard]] inline constexpr uint32_t Bit_scan_reverse(const uint32_t Value) noexcept {
		...
		[[nodiscard]] inline constexpr uint32_t Bit_scan_reverse(const uint64_t Value) noexcept {
			unsigned long Index; // Intentionally uninitialized for better codegen
		...
		[[nodiscard]] inline constexpr uint32_t Bit_scan_reverse(const Big_integer_flt& Xval) noexcept {
			if (Xval.Myused == 0) {
				return 0;
			}
		...
		[[nodiscard]] inline constexpr bool Shift_left(Big_integer_flt& Xval, const uint32_t Nx) noexcept {
			if (Xval.Myused == 0) {
				return true;
			}
		...
		[[nodiscard]] inline constexpr bool Add(Big_integer_flt& Xval, const uint32_t Value) noexcept {
			if (Value == 0) {
				return true;
			}
		...
		[[nodiscard]] inline constexpr uint32_t Add_carry(uint32_t& Ux1, const uint32_t Ux2, const uint32_t U_carry) noexcept {
			const uint64_t Uu = static_cast<uint64_t>(Ux1) + Ux2 + U_carry;
			Ux1 = static_cast<uint32_t>(Uu);
			return static_cast<uint32_t>(Uu >> 32);
		}

		[[nodiscard]] inline constexpr uint32_t Add_multiply_carry(
		...
		[[nodiscard]] inline constexpr uint32_t Multiply_core(
		...
		[[nodiscard]] inline constexpr bool Multiply(Big_integer_flt& Multiplicand, const uint32_t Multiplier) noexcept {
			if (Multiplier == 0) {
		...
		[[nodiscard]] inline constexpr bool Multiply(Big_integer_flt& Multiplicand, const Big_integer_flt& Multiplier) noexcept {
			if (Multiplicand.Myused == 0) {
				return true;
			}
		...
		inline constexpr uint32_t Large_power_data[] = { 0x540be400, 0x00000002, 0x63100000, 0x6bc75e2d, 0x00000005,
		...
		inline constexpr Unpack_index Large_power_indices[] = { { 0, 0, 2 }, { 2, 0, 3 }, { 5, 0, 4 }, { 9, 1, 4 }, { 13, 1, 5 },
		...
		inline constexpr uint32_t Small_powers_of_ten[9] = {
			10, 100, 1'000, 10'000, 100'000, 1'000'000, 10'000'000, 100'000'000, 1'000'000'000 };

		[[nodiscard]] inline constexpr bool Multiply_by_power_of_ten(Big_integer_flt& Xval, const uint32_t Power) noexcept {
		...
		[[nodiscard]] inline constexpr uint32_t Count_sequential_high_zeroes(const uint32_t Ux) noexcept {
			unsigned long Index; // Intentionally uninitialized for better codegen
			return BitScanReverse_constexpr(&Index, Ux) ? 31 - Index : 32;
		}

		[[nodiscard]] inline constexpr uint64_t Divide(Big_integer_flt& Numerator, const Big_integer_flt& Denominator) noexcept {
		...
		[[nodiscard]] inline constexpr uint64_t Right_shift_with_rounding(
		...
		inline constexpr void Accumulate_decimal_digits_into_big_integer_flt(
